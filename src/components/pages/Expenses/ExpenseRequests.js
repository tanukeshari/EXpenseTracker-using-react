...
export async function editExpense(id, item, dispatch) {
    const res = await axios.put(`https://expensetracker-28d72-default-rtdb.firebaseio.com/expenses.json/${id}.json`, item);
    await getExpenses(dispatch);
}

export async function postExpenses(item, dispatch){
    const res = await axios.post('https://expensetracker-28d72-default-rtdb.firebaseio.com/expenses.json',item);
    await getExpenses(dispatch)
}